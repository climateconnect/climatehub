...
- [ ] **AC-2**: A logged-out user goes through the existing sign-in redirect and, after signing in, lands back on the project page with the drawer open — both when arriving via the auto-open link and when clicking a contact entry point on a plain URL (in the latter case the contact click adds the auto-open flag to the redirect URL so the intent survives the round-trip; the flag is inserted before any tab hash).
...
- 2026-09-23 09:10 UTC — Production-readiness review of the committed POC (`859409641`, merged with master at `729b8bcd7` on branch `poc-inline-new-chat`). Four issues found and fixed (uncommitted): (1) **regression**: the drawer overwrote the app-wide `chatSocket.onmessage` (`_app.tsx` uses it to refresh notifications) — the drawer now chains the previous handler (notification refresh continues) and restores it on unmount/dep change, which also makes the handler correct under StrictMode remounts; covered by new tests (handler chained + previous called + restore on unmount + untouched before chat resolves). (2) send failures showed the wrong fallback copy ("chat could not be started") — new `could_not_send_message` EN/DE key, covered by test. (3) parity gap: no `beforeunload` unsent-message warning — added while the drawer is open and the composer is non-empty (same convention as the chat page's MessagingLayout). (4) socket-path robustness: `JSON.parse` guarded, `getMessageFromServer` result guarded. Full verification on the merged branch: 844/844 tests (master merge adjusts the baseline), `yarn lint`, `yarn tsc`, `yarn format` clean. **Live smoke test pending**: local dev server and backend were down during the review — run one final manual pass (contact click, auto-open URL, send a message) once servers are up. Known platform-level limitation, unchanged by this POC: on socket reconnect `_app.connect()` rebinds `onmessage`, which drops any bound handler (same for the chat page) — revisit at graduation together with the notification auto-read parity.
- 2026-09-23 09:45 UTC — Corner-case fix (user Q&A round): reopening the drawer now **refetches the latest page of messages** each time it opens — previously the history was fetched once per contact per page visit, so a reply arriving while the drawer was closed (websocket currently broken platform-wide) only appeared after a full page reload. Restructure: the start effect now only resolves the conversation (create-or-get + participants + roles, still once per contact — no duplicate `start_private_chat` calls); message loading lives in its own effect keyed on `open` + `chat_uuid`, so every open fetches page 1 fresh and pagination state resets (standard reopen behavior). Reopen refresh is silent — previously loaded messages stay visible until the fresh page arrives; the spinner only shows when there is nothing to display yet (initial load). Load failure on first open shows a new inline error (`could_not_load_messages` EN/DE); a refresh failure keeps the existing content. This also benefits future consumers (e.g. profile page as a quick way back into a conversation). Complementary platform task agreed separately: websocket repair incl. handler chaining in `_app.connect()`. Tests: 2 added (refetch on reopen without re-starting the chat; history-load error), 19 drawer/page cases total; full suite 846/846, lint/tsc/format clean.
- 2026-09-23 10:05 UTC — Bug found during user testing (AC-2 gap): clicking contact while logged out redirected to sign-in but the `redirect` URL carried no auto-open flag, so after login the drawer stayed closed (only the deep-link case preserved the flag). Fix: `handleClickContact` now builds the redirect URL via `getContactChatRedirectUrl` (adds `openContactChat=true` if absent, inserted before any `#tab` hash since the page keeps its active tab in the URL hash; a URL that already carries the flag is returned unchanged). After login the user lands on the project page with the flag and the existing auto-open effect opens the drawer. Tests added: logged-out click adds the flag; flag placed before the tab hash. Full suite 848/848, lint/tsc/format clean.
- 2026-09-23 10:15 UTC — Role wording decided by user (confirmed working in a live test that the logged-out contact click now opens the drawer after login): the partner's role label is now generic — **"Contact person" / "Ansprechpartner:in"** (new `contact_person` project text) — replacing the per-type labels in both the drawer header and the contact button's hover card (`ContactCreatorButton`), with no project/idea/event variant. The type-aware distinction remains only in the empty-conversation context term ("about the event/project 'x'"). Removes the type switch in `ProjectPageRoot` and the `creator?.role` fallback chain in `ContactCreatorButton` (which also eliminates a latent crash rendering a role object as `CardHeader.subheader`); the now-unused `responsible_person_event` key removed. The pre-existing `responsible_person_project/_idea/_org` keys are no longer referenced by this UI — cleanup candidate at graduation. Full suite 848/848, lint/tsc/format clean.
